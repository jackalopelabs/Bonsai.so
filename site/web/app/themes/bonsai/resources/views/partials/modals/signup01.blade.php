    ...
    <div class="modal-dialog pb-5" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">Sign up for @php the_field('tier_1_name', 2); @endphp <i class="mdi mdi-arrow-right mute ml-2"></i></h5>
        ...
        <div class="modal-body">
          @guest
            ...
            @if( get_field('tier_01_custom_guest') )
              @php the_field('tier_01_custom_guest', 2); @endphp
            @else
              @php
                $form_id = get_field('tier_01_form_guest');
          ...
          @endguest
          @role('subscriber')
            @if( get_field('tier_01_custom_subscriber') )
              @php the_field('tier_01_custom_subscriber', 2); @endphp
            @endif
            @php
              $form_id = get_field('tier_01_form_subscriber');
              gravity_form($form_id);
            @endphp
            @if( get_field('web3_cta') )
              <div class="mb-4" style="">
          ...
          @endrole
          @role('tier_01')
            @if( get_field('tier_01_custom_tier_01') )
              @php the_field('tier_01_custom_tier_01', 2); @endphp
            @endif
            @php
              $form_id = get_field('tier_01_form_tier_01');
              gravity_form($form_id);
            @endphp
            @if( get_field('web3_cta') )
              <div class="mb-4" style="">
          ...
          @endrole
          @role('tier_02')
          @if( get_field('tier_01_custom_tier_02') )
            @php the_field('tier_01_custom_tier_02', 2); @endphp
          @endif
          @php
            $form_id = get_field('tier_01_form_tier_02');
            gravity_form($form_id);
          @endphp
            @if( get_field('web3_cta') )
              <div class="mb-4" style="">
          ...
          @endrole
          @role('tier_03')
            @if( get_field('tier_01_custom_tier_03') )
              @php the_field('tier_01_custom_tier_03', 2); @endphp
            @endif
            @php
              $form_id = get_field('tier_01_form_tier_03');
              gravity_form($form_id);
            @endphp
            @if( get_field('web3_cta') )
              <div class="mb-4" style="">
